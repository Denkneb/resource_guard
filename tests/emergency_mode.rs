...
use resource_guard::{
    ...
    domain::{
        EmergencyAction, EmergencyPolicy, MemoryPressureLevel, MemoryPressurePolicy,
        MemoryPressureSample, MemoryPsi, ProtectionPolicy, SystemResources,
    },
};
...
fn artificial_critical_pressure_terminates_an_allowlisted_child() {
    ...
    let candidate = emergency
        .consider(evaluation.current, &[observed], Duration::ZERO, false)
        .unwrap();
